...
void LoadScene(Scene& scene)
{
	...
	// Load vertex shader
	{
		char metaData[] =
		{
			...
			0x00, 0x00, 0x00, 0x01,	// 1 constant buffers
			0x00, 0x00, 0x00, 0x00, // Constant buffer index 0
			0x00, 0x00, 0x00, 0x09,	// Name size is 9
			't', 'r', 'a', 'n', 's', 'f', 'o', 'r', 'm',

			0x00, 0x00, 0x00, 0x01, // 1 constant buffer vars
			0x00, 0x00, 0x00, 0x00, // On constant buffer on index 0
			0x00, 0x00, 0x00, 0x00, // Offset is 0
			0x00, 0x00, 0x00, 0x04, // Index is 4
			0x00, 0x00, 0x00, 0x03, // Name size is 3
			'm', 'v', 'p',
			0x00, 0x00, 0x00, 0x0D, // Var type is mat4 (0x10)
		};

		auto file = scene.fileSystem->OpenFile(Files::FileMode::Read, "/vertex3.mslbc");
		...
		file = scene.fileSystem->OpenFile(Files::FileMode::Write, "/vertex3.bc");
	...
	// Load pixel shader
	{
		...
		auto file = scene.fileSystem->OpenFile(Files::FileMode::Read, "/pixel3.mslbc");
		auto size = scene.fileSystem->GetSize(file);
		auto code = new char[size + 1];
		scene.fileSystem->Read(file, code, size);
		scene.fileSystem->CloseFile(file);
		...
		file = scene.fileSystem->OpenFile(Files::FileMode::Write, "/pixel3.bc");
		scene.fileSystem->Write(file, bytecode, bytecodeSize);
		scene.fileSystem->CloseFile(file);
