...
void Main(int argc, char** argv)
{
	...
	if (err != MFG_ERROR_OKAY)
		abort();
	mfsPrintFormatUTF8(mfsOut, u8"Successfully got input variable from the loaded meta data:\n- Type: %d\nIndex: %d\n", inputVar->type, inputVar->id);
	...
	if (err != MFG_ERROR_OKAY)
		abort();
	mfsPrintFormatUTF8(mfsOut, u8"Successfully got output variable from the loaded meta data:\n- Type: %d\nIndex: %d\n", outputVar->type, outputVar->id);
