...
Magma::Resources::ShaderImporter::ShaderImporter(Manager* manager, Framework::Graphics_V1X::RenderDevice* device)
...
void Magma::Resources::ShaderImporter::Import(Resource * resource)
{
	...
	if (path.GetExtension() == "msl")
	{
		...
		{
			char bo[4096];
			auto boSize = Framework::Graphics_V1X::ShaderCompiler::Run(src, bo, sizeof(bo));
			delete[] src;

			Framework::Graphics_V1X::ShaderData shaderData(bo, boSize);
			data->type = shaderData.GetShaderType();
			switch (data->type)
			{
				case Framework::Graphics_V1X::ShaderType::Vertex:
					data->vertexShader = m_device->CreateVertexShader(shaderData);
					break;

				case Framework::Graphics_V1X::ShaderType::Pixel:
					data->pixelShader = m_device->CreatePixelShader(shaderData);
					break;

				case Framework::Graphics_V1X::ShaderType::Invalid:
				{
					std::stringstream ss;
		...
		catch (Framework::Graphics_V1X::ShaderError& err)
		{
			std::stringstream ss;
			ss << "Failed to import shader resource on path '" << path.ToString() << "':" << std::endl;
