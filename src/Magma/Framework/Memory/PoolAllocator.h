...
/*
	Implementation of a pool32 allocator in C.
*/
...
extern "C"
{
	...
	/// <summary>
	///		Creates a new magma framework memory pool32 allocator.
	/// </summary>
	/// <param name="poolAllocator">Pointer to allocator pointer</param>
	/// <param name="desc">Pointer to pool32 allocator description</param>
	/// <returns>
	///		Returns MFM_ERROR_OKAY if there were no errors.
	///		Returns MFM_ERROR_ALLOCATION_FAILED if the function couldn't allocate memory for the pool32.
	///		Returns MFM_ERROR_INVALID_ARGUMENTS if the pool32 description or allocator pointers are NULL.
	/// </returns>
	...
	/// <param name="poolAllocator">Pointer to allocator pointer</param>
	/// <param name="desc">Pointer to pool32 allocator description</param>
	/// <param name="memory">Pointer to memory adress where the pool32 allocator will be created</param>
	/// <param name="memorySize">Size of the reserved memory</param>
	/// <returns>
	///		Returns MFM_ERROR_OKAY if there were no errors.
	///		Returns MFM_ERROR_INVALID_ARGUMENTS if the pool32 description or allocator pointers are NULL or if the pool32 is marked as expandable or if the pool32 doesn't fit on the memory passed.
	/// </returns>
	...
	/// <summary>
	///		Destroys a magma framework memory pool32 allocator.
	/// </summary>
	/// <param name="poolAllocator">Pool allocator to destroy</param>
	void mfmDestroyPoolAllocator(void * poolAllocator);

	/// <summary>
	///		Allocates on a magma framework memory pool32 allocator.
	/// </summary>
	/// <param name="allocator">Magma framework memory pool32 allocator</param>
	/// <param name="memory">Pointer to allocated memory pointer</param>
	/// <param name="size">Memory allocation size in bytes</param>
	/// <returns>
	///		Returns MFM_ERROR_OKAY if there were no errors.
	///		Returns MFM_ERROR_ALLOCATOR_OVERFLOW if the pool32 has no free slots.
	///		Returns MFM_ERROR_ALLOCATION_TOO_BIG if the allocation size is bigger than the pool32 slot's size.
	///		Returns MFM_ERROR_ALLOCATION_FAILED if the pool32 failed to expand.
	/// </returns>
	mfmError mfmPoolAllocate(mfmPoolAllocator* allocator, void** memory, mfmU64 size);

	/// <summary>
	///		Deallocates on a magma framework memory pool32 allocator.
	/// </summary>
	/// <param name="allocator">Magma framework memory pool32 allocator</param>
	/// <param name="memory">Pointer to the allocated memory</param>
	/// <returns>
	///		Returns MFM_ERROR_OKAY if there were no errors.
	///		Returns MFM_ERROR_OUT_OF_BOUNDS if the memory is out of the pool32 bounds.
	/// </returns>
	mfmError mfmPoolDeallocate(mfmPoolAllocator* allocator, void* memory);

	/// <summary>
	///		Gets the number of slots in a pool32 allocator.
	/// </summary>
	...
	/// <summary>
	///		Gets the number of chunks in a pool32 allocator.
	/// </summary>
	/// <param name="allocator">Allocator pointer</param>
	...
	/// <summary>
	///		Gets the number of occupied slots in a pool32 allocator.
	/// </summary>
	/// <param name="allocator">Allocator pointer</param>
