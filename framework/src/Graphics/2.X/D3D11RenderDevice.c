...
void mfgD3D11DestroyVertexShader(void* vs)
{
	...
	if (mfmReleaseObject(d3dVS->base.renderDevice) != MF_ERROR_OKAY)
		abort();
	if (mfmReleaseObject(d3dVS->md) != MF_ERROR_OKAY)
		abort();
	if (mfmDeinitObject(&d3dVS->base.object) != MF_ERROR_OKAY)
		abort();
...
void mfgD3D11DestroyPixelShader(void* ps)
{
	...
	if (mfmReleaseObject(d3dPS->base.renderDevice) != MF_ERROR_OKAY)
		abort();
	if (mfmReleaseObject(d3dPS->md) != MF_ERROR_OKAY)
		abort();
	if (mfmDeinitObject(&d3dPS->base.object) != MF_ERROR_OKAY)
		abort();
...
mfError mfgD3D11BindConstantBuffer(mfgV2XRenderDevice* rd, mfgV2XBindingPoint* bp, mfgV2XConstantBuffer* cb)
{
	...
	{
		err = mfmReleaseObject(d3dBP->boundObject);
		if (err != MF_ERROR_OKAY)
			return err;
	}
	...
	if (d3dBP->shaderType == MFG_VERTEX_SHADER)
	{
		...
		{
			err = mfmAcquireObject(d3dBP->boundObject);
			if (err != MF_ERROR_OKAY)
				return err;
...
mfError mfgD3D11BindSampler(mfgV2XRenderDevice* rd, mfgV2XBindingPoint* bp, mfgV2XSampler* sampler)
{
	...
	{
		err = mfmReleaseObject(d3dBP->boundSampler);
		if (err != MF_ERROR_OKAY)
			return err;
	}
	...
	if (d3dBP->shaderType == MFG_VERTEX_SHADER)
	{
		...
		{
			err = mfmAcquireObject(d3dBP->boundSampler);
			if (err != MF_ERROR_OKAY)
				return err;
...
void mfgD3D11DestroyPipeline(void* pp)
{
	...
	mfError err = mfmReleaseObject(&d3dPP->vertex->base.object);
	if (err != MF_ERROR_OKAY)
		return err;
	err = mfmReleaseObject(&d3dPP->pixel->base.object);
	if (err != MF_ERROR_OKAY)
		return err;
	if (mfmReleaseObject(d3dPP->base.renderDevice) != MF_ERROR_OKAY)
		abort();
	if (mfmDeinitObject(&d3dPP->base.object) != MF_ERROR_OKAY)
		abort();
...
mfError mfgD3D11CreatePipeline(mfgV2XRenderDevice* rd, mfgV2XPipeline** pp, mfgV2XVertexShader* vs, mfgV2XPixelShader* ps)
{
	...
	err = mfmAcquireObject(&d3dPP->vertex->base.object);
	if (err != MF_ERROR_OKAY)
		return err;
	err = mfmAcquireObject(&d3dPP->pixel->base.object);
	if (err != MF_ERROR_OKAY)
		return err;
...
mfError mfgD3D11SetPipeline(mfgV2XRenderDevice* rd, mfgV2XPipeline* pp)
{
	...
	{
		err = mfmReleaseObject(d3dRD->currentPipeline);
		if (err != MF_ERROR_OKAY)
			return err;
	}
...
void mfgD3D11DestroyConstantBuffer(void* buffer)
{
	...
	if (mfmReleaseObject(d3dCB->base.renderDevice) != MF_ERROR_OKAY)
		abort();
	if (mfmDeinitObject(&d3dCB->base.object) != MF_ERROR_OKAY)
		abort();
...
mfError mfgD3D11CreateConstantBuffer(mfgV2XRenderDevice* rd, mfgV2XConstantBuffer** cb, mfmU64 size, const void* data, mfgEnum usage)
{
	...
	*cb = d3dCB;
	mfError err = mfmAcquireObject(rd);
	if (err != MF_ERROR_OKAY)
		MFG_RETURN_ERROR(err, u8"mfmIncObjectRef failed on render device");
...
void mfgD3D11DestroyVertexBuffer(void* buffer)
{
	...
	if (mfmReleaseObject(d3dVB->base.renderDevice) != MF_ERROR_OKAY)
		abort();
	if (mfmDeinitObject(&d3dVB->base.object) != MF_ERROR_OKAY)
		abort();
...
void mfgD3D11DestroyIndexBuffer(void* buffer)
{
	...
	if (mfmReleaseObject(d3dIB->base.renderDevice) != MF_ERROR_OKAY)
		abort();
	if (mfmDeinitObject(&d3dIB->base.object) != MF_ERROR_OKAY)
		abort();
...
mfError mfgD3D11SetIndexBuffer(mfgV2XRenderDevice* rd, mfgV2XIndexBuffer* ib)
{
	...
	{
		err = mfmReleaseObject(d3dRD->currentIndexBuffer);
		if (err != MF_ERROR_OKAY)
			return err;
	}
	...
	{
		err = mfmAcquireObject(d3dRD->currentIndexBuffer);
		if (err != MF_ERROR_OKAY)
			return err;
...
void mfgD3D11DestroyVertexLayout(void* vl)
{
	...
	if (mfmReleaseObject(d3dVL->base.renderDevice) != MF_ERROR_OKAY)
		abort();
	if (mfmDeinitObject(&d3dVL->base.object) != MF_ERROR_OKAY)
		abort();
...
void mfgD3D11DestroyVertexArray(void* va)
{
	...
	if (mfmReleaseObject(d3dVA->layout) != MF_ERROR_OKAY)
		abort();
	for (mfmU64 i = 0; i < 16; ++i)
	{
		if (d3dVA->vb[i] == NULL)
			continue;
		if (mfmReleaseObject(d3dVA->vb[i]) != MF_ERROR_OKAY)
			abort();
	}
	if (mfmReleaseObject(d3dVA->base.renderDevice) != MF_ERROR_OKAY)
		abort();
	if (mfmDeinitObject(&d3dVA->base.object) != MF_ERROR_OKAY)
		abort();
...
mfError mfgD3D11CreateVertexArray(mfgV2XRenderDevice* rd, mfgV2XVertexArray** va, mfmU64 bufferCount, mfgV2XVertexBuffer** buffers, mfgV2XVertexLayout* vl)
{
	...
	for (mfmU64 i = 0; i < bufferCount; ++i)
	{
		...
		err = mfmAcquireObject(d3dVA->vb[i]);
		if (err != MF_ERROR_OKAY)
			return err;
...
mfError mfgD3D11SetVertexArray(mfgV2XRenderDevice* rd, mfgV2XVertexArray* va)
{
	...
	{
		err = mfmReleaseObject(d3dRD->currentVertexArray);
		if (err != MF_ERROR_OKAY)
			return err;
	}
	...
	{
		err = mfmAcquireObject(d3dRD->currentVertexArray);
		if (err != MF_ERROR_OKAY)
			return err;
...
void mfgD3D11DestroyTexture1D(void* tex)
{
	...
	if (mfmReleaseObject(d3dTex->base.renderDevice) != MF_ERROR_OKAY)
		abort();
	if (mfmDeinitObject(&d3dTex->base.object) != MF_ERROR_OKAY)
		abort();
...
void mfgD3D11DestroySampler(void* sampler)
{
	...
	if (mfmReleaseObject(d3dS->base.renderDevice) != MF_ERROR_OKAY)
		abort();
	if (mfmDeinitObject(&d3dS->base.object) != MF_ERROR_OKAY)
		abort();
...
void mfgD3D11DestroyFramebuffer(void* fb)
{
	...
	for (mfmU8 i = 0; i < 8; ++i)
		if (d3dFB->textures[i] != NULL)
			if (mfmReleaseObject(d3dFB->textures[i]) != MF_ERROR_OKAY)
				abort();
	if (d3dFB->depthStencilTexture != NULL)
		if (mfmReleaseObject(d3dFB->depthStencilTexture) != MF_ERROR_OKAY)
			abort();
	if (mfmReleaseObject(d3dFB->base.renderDevice) != MF_ERROR_OKAY)
		abort();
	if (mfmDeinitObject(&d3dFB->base.object) != MF_ERROR_OKAY)
		abort();
...
mfError mfgD3D11CreateFramebuffer(mfgV2XRenderDevice* rd, mfgV2XFramebuffer** fb, mfmU64 textureCount, mfgV2XRenderTexture** textures, mfgV2XDepthStencilTexture* depthStencilTexture)
{
	...
	for (mfmU64 i = 0; i < textureCount; ++i)
	{
		...
		err = mfmAcquireObject(textures[i]);
		if (err != MF_ERROR_OKAY)
			return err;
	}
	...
	else
	{
		...
		err = mfmAcquireObject(depthStencilTexture);
		if (err != MF_ERROR_OKAY)
			return err;
...
void mfgD3D11DestroyRasterState(void* state)
{
	...
	if (rs != ((mfgD3D11RenderDevice*)rs->base.renderDevice)->defaultRasterState)
		if (mfmReleaseObject(rs->base.renderDevice) != MF_ERROR_OKAY)
			abort();
	if (mfmDeinitObject(&rs->base.object) != MF_ERROR_OKAY)
		abort();
...
mfError mfgD3D11SetRasterState(mfgV2XRenderDevice* rd, mfgV2XRasterState* state)
{
	...
	if (state != d3dRD->defaultRasterState)
	{
		...
		{
			err = mfmReleaseObject(((mfgD3D11RenderDevice*)rd)->currentRasterState);
			if (err != MF_ERROR_OKAY)
				return err;
		}

		err = mfmAcquireObject(state);
		if (err != MF_ERROR_OKAY)
			return err;
	}
...
void mfgD3D11DestroyDepthStencilState(void* state)
{
	...
	if (dss != ((mfgD3D11RenderDevice*)dss->base.renderDevice)->defaultDepthStencilState)
		if (mfmReleaseObject(dss->base.renderDevice) != MF_ERROR_OKAY)
			abort();
	if (mfmDeinitObject(&dss->base.object) != MF_ERROR_OKAY)
		abort();
...
mfError mfgD3D11SetDepthStencilState(mfgV2XRenderDevice* rd, mfgV2XDepthStencilState* state)
{
	...
	if (state != d3dRD->defaultDepthStencilState)
	{
		...
		{
			err = mfmReleaseObject(((mfgD3D11RenderDevice*)rd)->currentDepthStencilState);
			if (err != MF_ERROR_OKAY)
				return err;
		}
...
void mfgD3D11DestroyBlendState(void* state)
{
	...
	if (bs != ((mfgD3D11RenderDevice*)bs->base.renderDevice)->defaultBlendState)
		if (mfmReleaseObject(bs->base.renderDevice) != MF_ERROR_OKAY)
			abort();
	if (mfmDeinitObject(&bs->base.object) != MF_ERROR_OKAY)
		abort();
...
mfError mfgD3D11SetBlendState(mfgV2XRenderDevice* rd, mfgV2XBlendState* state)
{
	...
	if (state != d3dRD->defaultBlendState)
	{
		...
		{
			err = mfmReleaseObject(((mfgD3D11RenderDevice*)rd)->currentBlendState);
			if (err != MF_ERROR_OKAY)
				return err;
		}
...
mfError mfgV2XCreateD3D11RenderDevice(mfgV2XRenderDevice ** renderDevice, mfiWindow* window, const mfgV2XRenderDeviceDesc * desc, void * allocator)
{
	...
	if (mfmAcquireObject(rd->defaultRasterState) != MF_ERROR_OKAY)
		abort();
	if (mfmAcquireObject(rd->defaultDepthStencilState) != MF_ERROR_OKAY)
		abort();
	if (mfmAcquireObject(rd->defaultBlendState) != MF_ERROR_OKAY)
		abort();

	if (mfmAcquireObject(rd->window) != MF_ERROR_OKAY)
		abort();
...
void mfgV2XDestroyD3D11RenderDevice(void * renderDevice)
{
	...
	if (mfmReleaseObject(rd->window) != MF_ERROR_OKAY)
		abort();
	...
	if (mfmReleaseObject(rd->defaultRasterState) != MF_ERROR_OKAY)
		abort();
	if (mfmReleaseObject(rd->defaultDepthStencilState) != MF_ERROR_OKAY)
		abort();
	if (mfmReleaseObject(rd->defaultBlendState) != MF_ERROR_OKAY)
		abort();
